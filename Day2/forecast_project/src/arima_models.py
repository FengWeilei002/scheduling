...
def arima_sigma(history: np.ndarray) -> np.ndarray:
    h = np.asarray(history, dtype=float)
    if h.shape[0] < 2:
        return np.std(h, axis=0).astype(np.float32)
    return np.std(h[1:] - h[:-1], axis=0).astype(np.float32)
