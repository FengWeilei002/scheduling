...
def build_encoder_decoder(model_type: str, seq_length: int, horizon: int, past_dim: int, time_dim: int, V: int) -> tf.keras.Model:
    ...
    out = layers.TimeDistributed(layers.Dense(V), name="y_out")(x)
...
class DeepForecastResult:
    forecast: np.ndarray              # (horizon, V) original scale placeholder
    val_sigma: np.ndarray             # (V,) original scale residual std (validation)
    model: Any
...
def train_deep_model(
    df_history: pd.DataFrame,       # date + V columns
    seq_length: int,
    horizon: int,
    train_split: float,
    val_split: float,
    ...
    cols = [c for c in df_history.columns if c != "date"]
    V = len(cols)
    y = df_history[cols].values.astype(np.float32)

    dates_hist = pd.DatetimeIndex(pd.to_datetime(df_history["date"]))
    tf_hist = make_time_features(dates_hist)
    ...
    n = X_past.shape[0]
    if n < 10:
        raise ValueError(f"Not enough training samples for deep model. Need more history or smaller seq/horizon. n={n}")

    tr, va = split_by_ratio(n, train_split, val_split)

    past_dim = V + time_dim
    model = build_encoder_decoder(model_type, seq_length, horizon, past_dim, time_dim, V)
    ...
    val_true = Y_lab[va]
    sigma_scaled = np.std((val_true - val_pred).reshape(-1, V), axis=0)
...
def deep_forward(
    res: DeepForecastResult,
    y_hist: np.ndarray,             # (T, V) original scale
    dates_hist: pd.DatetimeIndex,
    dates_fut: pd.DatetimeIndex,    # length H
    seq_length: int,
) -> np.ndarray:
    ...
    tf_hist = all_tf[:T]
    tf_fut = all_tf[T:]  # (H, time_dim)

    y_scaled = np.zeros_like(y_hist, dtype=np.float32)
    for j, sc in enumerate(res.scalers):
        y_scaled[:, j] = sc.transform(y_hist[:, j:j+1]).ravel().astype(np.float32)
    ...
    pred_scaled = res.model.predict([past_input, fut_tf_input], verbose=0)[0]
