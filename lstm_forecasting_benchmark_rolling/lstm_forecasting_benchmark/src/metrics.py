...
def mase_scale(y_insample: np.ndarray, m: int = 1, eps: float = 1e-8) -> np.ndarray:
    """
    Scale for MASE: mean absolute error of in-sample naive(m) forecasts.
    For non-seasonal data, m=1.
    Returns shape (V,) for multivariate.
    """
    ...
    diff = np.abs(y[m:] - y[:-m])
    scale = np.mean(diff, axis=0)
    scale = np.maximum(scale, eps)
    return scale


def evaluate_multivariate(
    y_true: np.ndarray,  # (H, V)
    y_pred: np.ndarray,  # (H, V)
    model_name: str,
    ...
    for j, v in enumerate(var_names):
        ...
        rows.append({
            ...
            "sMAPE": smape(y_true[:, j], y_pred[:, j]),
            "WAPE": wape(y_true[:, j], y_pred[:, j]),
            "MASE": (float(_mae / scale[j]) if scale is not None and np.isfinite(scale[j]) else np.nan),
        })
...
def evaluate_rolling(
    ...
    model_name: str,
    var_names=("Yd", "TSE", "TSEC"),
    y_insample: np.ndarray | None = None,  # (T_hist, V)
    mase_m: int = 1,
) -> pd.DataFrame:
    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)
